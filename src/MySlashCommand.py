...
class MySlashCommand(Extension):
    ...
    async def get_day(self, ctx: SlashContext, jour: str, personne: User = None) -> None:
        ...
        await get_tool(self.bot, ctx.guild).get_day_bt(ctx, jour, False, personne)
    ...
    async def today(self, ctx: SlashContext) -> None:
        ...
        await get_tool(self.bot, ctx.guild).get_day_bt(ctx, date.today().strftime("%d-%m-%Y"), False)
    ...
    async def tomorrow(self, ctx: SlashContext) -> None:
        ...
        await get_tool(self.bot, ctx.guild).get_day_bt(ctx, (date.today() + timedelta(days=1)).strftime("%d-%m-%Y"), False)
    ...
    async def get_week(self, ctx: SlashContext, semaine: str, personne: User = None) -> None:
        ...
        await get_tool(self.bot, ctx.guild).get_week_bt(ctx, semaine, False, personne)
    ...
    async def help(self, ctx: SlashContext) -> None:
        ...
        if get_tool(self.bot, ctx.guild).is_guild_chan(ctx.author):
            ephemeral = not ctx.author.has_role(get_bd_serveur(self.bot).get_roles(ctx.guild)[RoleEnum.PERMA]) # Permanent si la personne possédant le rôle
    ...
    async def ics(self, ctx: SlashContext, debut: str, fin: str) -> None:
        ...
        try:
            ...
            get_ics(filter_events(get_calendar(get_bd_serveur(self.bot).get_serveur(ctx.guild).annee).get_events(),
                                  [TimeFilter(date_debut, Timing.AFTER), TimeFilter(date_fin, Timing.BEFORE),
                                   get_tool(self.bot, ctx.guild).get_filiere(ctx.author), get_tool(self.bot, ctx.guild).get_groupes(ctx.author)]),
                    filename=filename
                    )

            ephemeral = False
            if get_tool(self.bot, ctx.guild).is_guild_chan(ctx.author):
                ephemeral = not ctx.author.has_role(get_bd_serveur(self.bot).get_roles(ctx.guild)[RoleEnum.PERMA])  # Permanent si la personne a le rôle
            ...
            await ctx.send(embeds=[get_tool(self.bot, ctx.guild).create_error_embed(f"La valeur `{debut}` ou `{fin}` ne correspond pas à une date.")], ephemeral=True)
    ...
    async def subscribe(self, ctx: SlashContext, service: str = "None", ics: str = "None") -> None:
        ...
        if not user_base.has_user(ctx.author_id):
            user_base.add_user(id, get_tool(self.bot, ctx.guild).get_groupes_as_list(ctx.author), get_tool(self.bot, ctx.guild).get_filiere_as_filiere(ctx.author))
        match service:
            case "DAILY":
                user_base.user_subscribe(id, Subscription.DAILY)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.DAILY, True)
            case "WEEKLY":
                user_base.user_subscribe(id, Subscription.WEEKLY)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.WEEKLY, True)
            case "BOTH":
                user_base.user_subscribe(id, Subscription.BOTH)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.BOTH, True)

        match ics:
            case "DAILY":
                user_base.user_subscribe_ics(id, Subscription.DAILY_ICS)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.DAILY_ICS, True)
            case "WEEKLY":
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.WEEKLY_ICS, True)
                user_base.user_subscribe_ics(id, Subscription.WEEKLY_ICS)
            case "BOTH":
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.BOTH_ICS, True)
                user_base.user_subscribe_ics(id, Subscription.BOTH_ICS)

        await get_tool(self.bot, ctx.guild).check_subscription(ctx)
    ...
    async def unsubscribe(self, ctx: SlashContext, service: str = "None", ics: str = "None") -> None:
        ...
        match service:
            case "DAILY":
                ...
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.DAILY, False)
            case "WEEKLY":
                user_base.user_unsubscribe(id, Subscription.WEEKLY)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.WEEKLY, False)
            case "BOTH":
                user_base.user_unsubscribe(id, Subscription.BOTH)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.BOTH, False)

        match ics:
            case "DAILY":
                user_base.user_unsubscribe_ics(id, Subscription.DAILY_ICS)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.DAILY_ICS, False)
            case "WEEKLY":
                user_base.user_unsubscribe_ics(id, Subscription.WEEKLY_ICS)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.WEEKLY_ICS, False)
            case "BOTH":
                user_base.user_unsubscribe_ics(id, Subscription.BOTH_ICS)
                await get_tool(self.bot, ctx.guild).subscription_role(id, Subscription.BOTH_ICS, False)
        await get_tool(self.bot, ctx.guild).check_subscription(ctx)
    ...
    async def exam(self, ctx: SlashContext) -> None:
        ...
        exams = get_calendar(get_bd_serveur(self.bot).get_serveur(ctx.guild).annee).get_exams()
    ...
    async def info(self, ctx: SlashContext, personne: User = None):
        ...
        str_role = ""
        for groupe in get_tool(self.bot, ctx.guild).get_groupes_as_list(author):
            str_role += groupe.value + ", "
        str_role = str_role.removesuffix(", ")
        await ctx.send(
            f"{"Vous êtes" if you_are else "Informations sur"} {author.display_name}!\n{"Votre" if you_are else "Sa"} filière est {get_tool(self.bot, ctx.guild).get_filiere_as_filiere(ctx.author).value} et {"vos" if you_are else "ses"} groupes {"sont" if len(get_tool(self.bot, ctx.guild).get_groupes_as_list(ctx.author)) > 1 else "est"} {str_role}.")